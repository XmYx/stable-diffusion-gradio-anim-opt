...
from torch import tensor, Tensor, float32, sqrt, cat, no_grad, randn, randn_like, long, full
import numpy as np
...
class DDIMSampler(object):
    ...
    def register_buffer(self, name, attr):
        if type(attr) == Tensor:
            if attr.device != torch_device("cuda"):
                attr = attr.to(torch_device("cuda"))
    ...
    def make_schedule(
        ...
        def to_torch(x): return x.clone().detach().to(float32).to(self.model.device)
        ...
        sigmas_for_original_sampling_steps = ddim_eta * sqrt(
    ...
    def ddim_sampling(
        ...
        if t_start is not None and t_start < wanted_steps:
            latent_img = self.stochastic_encode(
                x0, tensor([t_start]*batch_size).to(device))
            timesteps = timesteps[:t_start]

        if latent_img is None:
            latent_img = randn(shape, device=device)
    ...
    def p_sample_ddim(
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            ...
            x_in = cat([x] * 2)
            t_in = cat([t] * 2)
            c_in = cat([unconditional_conditioning, c])
        ...
        a_t     = full((b, 1, 1, 1), alphas[index],      device=device)
        a_prev  = full((b, 1, 1, 1), alphas_prev[index], device=device)
        sigma_t = full((b, 1, 1, 1), sigmas[index],      device=device)
        sqrt_one_minus_at = full(
    ...
    def stochastic_encode(self, x0, t, noise=None):
        ...
        sqrt_alphas_cumprod = sqrt(self.ddim_alphas)
