...
from helpers import save_samples, sampler_fn, DepthModel
...
def transform_image_3d(prev_img_cv2, adabins_helper, midas_model, midas_transform, rot_mat, translate, midas_weight, near_plane, far_plane, fov, sampling_mode, padding_mode):
    ...
    return result
"""
def transform_image_3d(prev_img_cv2, depth_tensor, rot_mat, translate, midas_weight, near_plane, far_plane, fov, sampling_mode, padding_mode):
    # adapted and optimized version of transform_image_3d from Disco Diffusion https://github.com/alembics/disco-diffusion
    w, h = prev_img_cv2.shape[1], prev_img_cv2.shape[0]
                    ...
                    if samplern in ["klms","dpm2","dpm2_ancestral","heun","euler","euler_ancestral"]:
                        ...
                        if samplern == 'ddim':
                            ...
                            samples = sampler.ddim_sampling(
                                c,               # [1, 77, 768] c
                                shape,  # (1, 4, 64, 64) z_enc
                                x0=init_latent,            # [1, 4, 64, 64] init_image
                                mask=mask,  # [1, 4, 64, 64] mask
                                # 12 (if 20 steps and strength 0.75 => 15)
                                t_start=t_start, #t_enc
                                unconditional_guidance_scale=scale,
                                # [1, 77, 768]
                                unconditional_conditioning=uc,
                            )  # [1, 4, 64, 64]
                            """
                            samples = sampler.ddim_sampling(z_enc,
                                                             c,
                                                             t_enc,
                                                             unconditional_guidance_scale=scale,
                                                             unconditional_conditioning=uc)
...
def anim(animation_mode, animation_prompts, key_frames,
        ...
        use_seq, seqlist, seqname, fixed_code):
            opt.pre_gobig_gfpgan = False
            opt.gobig = False
            ...
            def anim_frame_warp_3d(prev_img_cv2, translation_x_series, translation_y_series, translation_z_series, rotation_3d_x_series, rotation_3d_y_series, rotation_3d_z_series, frame_idx, depth):
                ...
                result = transform_image_3d(prev_img_cv2, depth, rot_mat, translate_xyz, midas_weight, near_plane, far_plane, fov, sampling_mode, padding_mode)
                torch.cuda.empty_cache()
                return result
                        ...
                        for frame_idx in range(start_frame,max_frames):
                            ...
                            if prev_sample is not None:
                                ...
                                if animation_mode == '2D':
                                    ...
                                    prev_img_cv2 = sample_to_cv2(prev_sample)    #prev_img = anim_frame_warp_3d(sample_to_cv2(prev_sample), translation_x_series, translation_y_series, translation_z_series, rotation_3d_x_series, rotation_3d_y_series, rotation_3d_z_series, frame_idx, adabins_helper, midas_model, mtransform)
                                    depth = depth_model.predict(prev_img_cv2, anim_args)
                                    mask_file = os.path.join(outdir, f"depth_{timestring}_{frame_idx-1:05}.png")
                                    """
                                    depth_min = 1000
                                    depth_max = -1000
                                    ...
                                    depth_model.save(mask_file, depth)
                                    use_mask = True

                                    prev_img = anim_frame_warp_3d(prev_img_cv2, translation_x_series, translation_y_series, translation_z_series, rotation_3d_x_series, rotation_3d_y_series, rotation_3d_z_series, frame_idx, depth)
...
with demo:
    with gr.Tabs():
            ...
            with gr.Row():
                with gr.Column(scale=3):
                    ...
                    with gr.Row():
                        ...
                        invert_mask = gr.Checkbox(label='Invert Mask', value=False, interactive=True) #@param {type:"boolean"}
                        use_mask = gr.Checkbox(label='Use Depth as Mask', value=False, interactive=True) #@param {type:"boolean"}
                        fixed_code = gr.Checkbox(label='Fixed Code', value=True, interactive=True) #@param {type:"boolean"}
                        with gr.Row():
                ...
                with gr.Column(scale=4):
                    ...
                    with gr.TabItem('Video / Init Video / Interpolation settings'):
                      ...
                      strength = gr.Slider(minimum=0, maximum=1, step=0.1, label='Init Image Strength', value=1.0)#strength
        ...
        with gr.TabItem('InPainting'):
            with gr.Row():
                with gr.Column():
                    with gr.Row():
                        i_maskmode = gr.Radio(label="crop mode", choices=['uncrop', 'inpaint'], interactive=True, value='inpaint', visible=True)
    ...
    anim_inputs = [animation_mode, new_k_prompts, key_frames,
                    ...
                    use_seq, sequence, seqname, fixed_code]
