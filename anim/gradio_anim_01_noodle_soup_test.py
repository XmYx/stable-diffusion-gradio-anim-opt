...
def sample_model(input_im, model, sampler, precision, h, w, ddim_steps, n_samples, scale, ddim_eta):
    ...
    with precision_scope("cuda"):
        with model.ema_scope():
            ...
            samples_ddim = K.sampling.__dict__[f'sample_{sampler}'](model_wrap_cfg, x, sigmas, extra_args={'cond': c, 'uncond': uc, 'cond_scale': cfg_scale}, disable=False)
...
def variations(input_im, outdir):
    ...
    config_var="/content/sd_vars/stable_diffusion/configs/stable-diffusion/sd-image-condition-finetune.yaml",
    outpath=outdir,
    scale=3.0,
    h=512,
    w=512,
    ...
    model_var = load_model_from_config(config_var, ckpt_var, device=device)
...
with demo:
    with gr.Tabs():
        ...
        with gr.TabItem('NoodleSoup'):
            with gr.Column():
                input_prompt = gr.Textbox(label='IN',  placeholder='Prompt', lines=2)
                output_prompt = gr.Textbox(label='OUT',  placeholder='Prompt', lines=5)
                soup_btn = gr.Button('Cook')

    var_inputs = [input_var]
    ...
    soup_btn.click(fn=, inputs=soup_inputs, outputs=soup_outputs)
