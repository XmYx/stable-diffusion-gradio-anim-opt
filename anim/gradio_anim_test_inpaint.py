...
def arger(animation_prompts, prompts, animation_mode, strength, max_frames, border, key_frames, interp_spline, angle, zoom, translation_x, translation_y, translation_z, color_coherence, previous_frame_noise, previous_frame_strength, video_init_path, extract_nth_frame, interpolate_x_frames, batch_name, outdir, save_grid, save_settings, save_samples, display_samples, n_samples, W, H, init_image, seed, sampler, steps, scale, ddim_eta, seed_behavior, n_batch, use_init, timestring, noise_schedule, strength_schedule, contrast_schedule, resume_from_timestring, resume_timestring, make_grid, GFPGAN, bg_upsampling, upscale, rotation_3d_x, rotation_3d_y, rotation_3d_z, use_depth_warping, midas_weight, near_plane, far_plane, fov, padding_mode, sampling_mode, init_img_array, use_mask, mask_file, invert_mask, mask_brightness_adjust, mask_contrast_adjust):
    print(f'Image Mask: {init_img_array}')
    init_img_array = init_img_array
    precision = 'autocast'
    fixed_code = True
    C = 4
    f = 8
...
def generate(args, init_img_array, return_latent=False, return_sample=False, return_c=False):
    ...
    elif args.init_img_array is not None:
        print(args.init_img_array)
        init_image = init_img_array['image']
        shape=(args.W, args.H)
        init_image = init_image.resize(shape, resample=Image.LANCZOS)
        init_image = np.array(init_image).astype(np.float16) / 255.0
        init_image = init_image[None].transpose(0, 3, 1, 2)
        init_image = torch.from_numpy(init_image).to(device)

        print("Init Image Found")
        print(init_image)
    ...
    if args.use_mask:
        ...
        #assert args.image_mask is not None, "use_mask==True: An mask image is required for a mask"
        #assert args.use_init, "use_mask==True: use_init is required for a mask"
        #assert init_latent is not None, "use_mask==True: An latent init image is required for a mask"
        mask = prepare_mask(init_img_array['mask'],
                            init_latent.shape,
...
def anim(animation_mode: str, animation_prompts: str, key_frames: bool, prompts: str, batch_name: str, outdir: str, max_frames: int, GFPGAN: bool, bg_upsampling: bool, upscale: int, W: int, H: int, steps: int, scale: int, angle: str, zoom: str, translation_x: str, translation_y: str, translation_z: str, rotation_3d_x: str, rotation_3d_y: str, rotation_3d_z: str, use_depth_warping: bool, midas_weight: float, near_plane: int, far_plane: int, fov: int, padding_mode: str, sampling_mode: str, seed_behavior: str, seed: str, interp_spline: str, noise_schedule: str, strength_schedule: str, contrast_schedule: str, sampler: str, extract_nth_frame: int, interpolate_x_frames: int, border: str, color_coherence: str, previous_frame_noise: float, previous_frame_strength: float, video_init_path: str, save_grid: bool, save_settings: bool, save_samples: bool, display_samples: bool, n_batch: int, n_samples: int, ddim_eta: float, use_init: bool, init_image: str, strength: float, timestring: str, resume_from_timestring: bool, resume_timestring: str, make_grid: bool, init_img_array, use_mask: bool, mask_file: str, invert_mask: bool, mask_brightness_adjust: float, mask_contrast_adjust: float):
    print(f'Image Mask: {init_img_array}')
    ...
    def render_animation(args):
        ...
        for frame_idx in range(start_frame,args.max_frames):
            ...
            results = generate(args, init_img_array, return_latent=False, return_sample=True)
        ...
        for iprompt, prompt in enumerate(args.prompts):
            ...
            for batch_index in range(args.n_batch):
                ...
                for image in init_array: # iterates the init images
                    ...
                    results = generate(args, init_img_array)
    ...
    arger(animation_prompts, prompts, animation_mode, strength, max_frames, border, key_frames, interp_spline, angle, zoom, translation_x, translation_y, translation_z, color_coherence, previous_frame_noise, previous_frame_strength, video_init_path, extract_nth_frame, interpolate_x_frames, batch_name, outdir, save_grid, save_settings, save_samples, display_samples, n_samples, W, H, init_image, seed, sampler, steps, scale, ddim_eta, seed_behavior, n_batch, use_init, timestring, noise_schedule, strength_schedule, contrast_schedule, resume_from_timestring, resume_timestring, make_grid, GFPGAN, bg_upsampling, upscale, rotation_3d_x, rotation_3d_y, rotation_3d_z, use_depth_warping, midas_weight, near_plane, far_plane, fov, padding_mode, sampling_mode, image_mask, use_mask, mask_file, invert_mask, mask_brightness_adjust, mask_contrast_adjust)
...
with demo:
        ...
        with gr.TabItem('Animation'):
            with gr.Row():
                ...
                with gr.Column(scale=1.6):
                        ...
                        with gr.Row():
                            GFPGAN = gr.Checkbox(label='GFPGAN, Upscaler', value=False, interactive=True)
                            bg_upsampling = gr.Checkbox(label='BG Enhancement', value=False, interactive=True)
                    ...
                    with gr.TabItem('Movements'):
                        with gr.Row():
                            with gr.Column(scale=0.13):
                                angle = gr.Textbox(label='Angles',  placeholder='0:(0)', lines=1, value='0:(0)', interactive=True)#angle
                                zoom = gr.Textbox(label='Zoom',  placeholder='0: (1.04)', lines=1, value='0:(1.0)', interactive=True)#zoom
                                translation_x = gr.Textbox(label='Translation X (+ is Camera Left, large values [1 - 50])',  placeholder='0: (0)', lines=1, value='0:(0)', interactive=True)#translation_x
                                translation_y = gr.Textbox(label='Translation Y + = R',  placeholder='0: (0)', lines=1, value='0:(0)', interactive=True)#translation_y
                                translation_z = gr.Textbox(label='Translation Z + = FW',  placeholder='0: (0)', lines=1, value='0:(0)', visible=True, interactive=True)#translation_y
                                rotation_3d_x = gr.Textbox(label='Rotation 3D X (+ is Up)',  placeholder='0: (0)', lines=1, value='0:(0)', visible=True, interactive=True)#rotation_3d_x
                                rotation_3d_y = gr.Textbox(label='Rotation 3D Y (+ is Right)',  placeholder='0: (0)', lines=1, value='0:(0)', visible=True, interactive=True)#rotation_3d_y
                            with gr.Column(scale=0.13):
                                rotation_3d_z = gr.Textbox(label='Rotation 3D Z (+ is Clockwise)',  placeholder='0: (0)', lines=1, value='0:(0)', visible=True, interactive=True)#rotation_3d_z
                        ...
                        with gr.Row():
                            with gr.Column(scale=0.15):
                                animation_mode = gr.Dropdown(label='Animation Mode',
                                                                ...
                                                                value='3D',
                                                                interactive=True)#animation_mode

                                max_frames = gr.Slider(minimum=1, maximum=1000, step=1, label='Frames to render', value=100, interactive=True)#max_frames
                                seed_behavior = gr.Dropdown(label='Seed Behavior', choices=['iter', 'fixed', 'random'], value='iter', interactive=True)#seed_behavior
                                seed = gr.Number(label='Seed',  placeholder='SEED HERE', value='-1', interactive=True)#seed
            ...
            with gr.Row():
                ...
                with gr.Column():
                    ...
                    with gr.Column():
                        ...
                        use_mask = gr.Checkbox(label='Use Mask', value=True, visible=True)
                        invert_mask = gr.Checkbox(label='Invert Mask', value=False, visible=True)
                    mask_file = gr.Textbox(label='Mask Image link',  placeholder='https://cdn.pixabay.com/photo/2022/07/30/13/10/green-longhorn-beetle-7353749_1280.jpg', lines=1)
                    mask_brightness_adjust = gr.Slider(minimum=0, maximum=10, step=0.1, label='mask_brightness_adjust', value=1.0)#n_samples
                    mask_contrast_adjust = gr.Slider(minimum=0, maximum=10, step=0.1, label='mask_brightness_adjust', value=1.0)#n_samples
    ...
    resume_from_timestring, resume_timestring, make_grid, image_mask,
    use_mask, mask_file, invert_mask, mask_brightness_adjust, mask_contrast_adjust]
